...
class MinIOStorageException(AppException):
    ...
    def __init__(
        self,
        message: str = "Error storing file in MinIO",
...
class VideoDBException(AppException):
    ...
    def __init__(
        self, message: str = "Database error", original_error: str | None = None
    ):
        if original_error:
            message = f"{message}: {original_error}"
        super().__init__(message, status_code=500)
...
class VideoProcessingException(AppException):
    ...
    def __init__(
        self, message: str = "Error processing video", original_error: str | None = None
    ):
        if original_error:
            message = f"{message}: {original_error}"
        super().__init__(message, status_code=500)
...
class JobDBException(AppException):
    """Excepción para errores de base de datos relacionados con jobs"""
